...
export function EnhancedSimpleVoteButton({
  ...
  const { isInitialized, voteForSenator, getCandidateIdByStudentId } =
    useStudentIdWeb3();
    ...
    try {
      ...
      if (!isTokenValid) {
        toast({
          title: "Invalid token",
          description:
            "Your voting token is invalid or has expired. Please try again.",
          variant: "destructive",
          duration: 5000,
        });
      ...
      try {
        ...
        console.log(
          `Looking up blockchain candidate ID for student ID: ${studentId}`,
        );
        ...
        try {
          ...
          if (!blockchainCandidateId || blockchainCandidateId <= 0) {
            console.warn(
              `Initial lookup failed for student ID: ${studentId}, trying to register it now...`,
            );
            ...
            try {
              ...
              blockchainCandidateId = await getCandidateIdByStudentId(
                studentId,
                true,
              );
              console.log(
                `Successfully obtained candidate ID ${blockchainCandidateId} for student ID: ${studentId}`,
              );
              ...
              if (
                ...
                console.log(
                  "Candidate was already registered, trying to get ID one more time...",
                );
                blockchainCandidateId =
                  await getCandidateIdByStudentId(studentId);

                if (!blockchainCandidateId || blockchainCandidateId <= 0) {
                  throw new Error("Candidate exists but ID retrieval failed");
                }
                console.log(
                  `Retrieved existing candidate ID: ${blockchainCandidateId}`,
                );
              } else {
          ...
          console.log(
            `Found blockchain candidate ID: ${blockchainCandidateId} for student ID: ${studentId}`,
          );
        } catch (err: any) {
          console.error(
            "Error getting or registering candidate by student ID:",
            err,
          );
          ...
          if (
            errorReason.includes("no candidate found") ||
            errorString.includes("no candidate found")
          ) {
            errorMsg =
              "This candidate needs to be registered on the blockchain by an admin. Please contact the election administrator.";
            errorTitle = "Candidate registration required";
          } else if (errorString.includes("already registered")) {
            errorMsg =
              "This candidate is already registered but we couldn't retrieve their ID. Please try again later.";
          ...
          throw new Error(
            "Failed to get blockchain candidate ID: " + (err.message || err),
          );
        }
        ...
        const electionIdentifier = Math.floor(
          new Date(election.startDate).getTime() / 1000,
        );
        console.log(
          `Using election start timestamp ${electionIdentifier} as the blockchain identifier`,
        );

        console.log(
          `Voting for election using identifier: ${electionIdentifier}, candidate ID: ${blockchainCandidateId}`,
        );
        ...
        console.log(
          `About to vote using the timestamp identifier: ${electionIdentifier} for candidate ID: ${blockchainCandidateId}`,
        );

        // Now vote using the improved student ID web3 service with enhanced response
        const { success, txHash, voteCount } = await voteForSenator(
          electionIdentifier,
          blockchainCandidateId,
        );
        ...
        if (success) {
          ...
          console.log(
            `Vote success details: Transaction hash: ${txHash || "unknown"}, Vote count: ${voteCount || "pending verification"}`,
          );

          // Notify parent component with transaction hash and vote count if available
          if (onVoteSuccess) {
            onVoteSuccess(
              txHash || "blockchain-transaction-success",
              voteCount,
            );
          }
        } else {
